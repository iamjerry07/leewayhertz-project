...
const createUser = async function (req, res) {
  try {
    ...
    if (!isValidString(data.address)) {
      return res.status(400).send({ status: false, msg: "Enter an address" });
    }
...
const getUserByID = async function (req, res) {
  try {
    ...
    if (!checkId)
      return res
        .status(400)
        .send({ status: false, msg: " This User does not exist" });

    const findByID = await userModel.findById({ _id: id });
    return res.status(200).send({ status: true, msg: findByID });
  } catch (err) {
